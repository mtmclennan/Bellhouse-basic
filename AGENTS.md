# Bellhouse Page Build Rules

## General Bellhouse Rules

- Reuse existing shared components before creating new ones.
- Maintain the current SASS-based design system.
- Each section must have one distinct purpose.
- Do not add extra sections to compensate for weak content.
...
- Keep pages focused on clarity, usability, SEO structure, and conversion.
...
Every Bellhouse page should feel purposeful, credible, and specific to its audience.

Do not make pages longer by repeating the same message in different sections.
Do not create structure for its own sake.
Build pages that are clear, useful, and easy for real customers or contractors to act on.
