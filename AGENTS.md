...
## Site Architecture and Intent Routing Rules

### Core Architecture Rules

- Treat `/resources/` as the umbrella section for calculators and future resource content.
- Use one canonical calculator structure only.
- Preferred calculator route structure:
  - `/resources/calculators/`
  - `/resources/calculators/excavation/`
  - `/resources/calculators/gravel/`
  - `/resources/calculators/topsoil/`
- Do not maintain competing top-level calculator hubs if `/resources/` is the canonical section.
- Do not introduce duplicate crawlable routes for the same calculator content.
- Do not add redirects for pre-launch routes that were never live in production.

### Navigation and Discoverability Rules

- `/contractors` and `/service-areas` are first-class site sections and should be discoverable in primary navigation and footer navigation.
- The footer should act as useful secondary navigation, not just a minimal legal/footer block.
- Resources/calculators should be reachable from header, footer, and relevant service pages.
- Important pages should not rely only on contextual links to be discovered.

### Breadcrumb Rules

- Service pages, service-area pages, and resource/calculator pages should support consistent breadcrumb hierarchy.
- Visible breadcrumbs and structured breadcrumb data should match the actual page hierarchy.
- Do not create breadcrumb paths that imply nonexistent parent sections.

### Audience Routing Rules

Bellhouse content should help users self-sort into the right path quickly.
...
- Homepage, services index, and major hub pages should route users toward the right audience path where helpful.
- Do not keep mixed-audience pages overly broad if the page can be sharpened toward one stronger intent.
- If a page targets more than one audience, clarify those paths intentionally instead of blending them together vaguely.

### Calculator Routing Rules

- Calculator pages should not end at a generic quote CTA only.
- Calculator pages should provide relevant next steps for:
  - matching service pages
  - local/service-area context where appropriate
  - contractor/commercial paths where appropriate
- Local pages may include light planning-tool links where they genuinely support the page intent.
- Do not force the same calculator block onto every service-area page.

### Metadata and Canonical Rules

- Service pages should include complete metadata, not just title and meta description.
- Include canonical metadata and Open Graph metadata on key pages.
- Keep metadata consistent with actual audience, geography, and service intent.
- Resolve trailing-slash inconsistencies between sitemap URLs, canonical URLs, and internal links.
- Do not keep alternate route structures alive without a clear canonical strategy.

### Legacy and Weak-Page Rules

- When improving older pages, prioritize intent clarity over adding more sections.
- Do not preserve vague brochure-style copy just because the page already exists.
- If a page mixes distinct intents, consider splitting or repositioning the page instead of layering on more generic content.
- Replace dead-end location references with live local-page links or a link to `/service-areas`.
- If config-driven sections exist but are not rendered, either render them properly or simplify the config so it matches the real page output.

### Rural / Farm Path Rules

- Farm/rural pages should not feel like lightly edited residential pages.
- Septic, pond, demolition, and rural sitework pages should connect naturally to each other where relevant.
- The site should provide a clearer rural/farm path through navigation, internal linking, or both.

### Footer Rules

- Footer navigation should be grouped by real user paths or site sections, not kept as a flat list once the site structure expands.
- Prefer grouped footer headings such as Services, Service Areas, Contractors, Resources, and Company where appropriate.
- Contractor-relevant service links should be grouped under a Contractors heading in the footer when that improves discoverability.

### FAQ Rules for Non-Local Pages

- Do not force FAQ sections onto every page.
- Use FAQs only where the page naturally creates real questions that help users decide or move forward.
- FAQ content must answer real customer questions and should not be added as filler for length.
