...
import { calculatorConfigs, createCalculatorFormInput } from '../../config/calculators';
...
describe('buildCalculatorLeadData', () => {
  it('builds excavation lead data with correct labels, units, and message body', () => {
    ...
    expect(lead.results['Excavation volume']).toBe('100.00 m³');
    expect(lead.results['Loose material']).toBe('120.00 m³');
    expect(lead.results['Estimated weight']).toBe('239.8 metric tonnes');
    ...
    expect(lead.message).toContain('Excavation volume: 100.00 m³');
    expect(lead.message).toContain('Estimated weight: 239.8 metric tonnes');
  ...
  it('formats imperial feet/inches dimensions and imperial output units', () => {
    ...
    expect(lead.results['Excavation volume']).toMatch(/ yd³$/);
    expect(lead.results['Estimated weight']).toMatch(/ short tons$/);
  });
});
